    ...
    if (myPackageChanged) {
        ...
        if (versionStrMatch) {
            ...
            if (patch >= 0) {
                ...
                const updatedPackageJson = packageJson.replace(`"version": "0.0.${patch}"`, `"version": "0.0.${newPatch}"`);
...
async function hasPackageChangedInLastCommit(packageName) {
    const changedFilesInPackage = parseInt(await execCommand(`echo "$(git diff HEAD~1 --name-only | grep '^packages/${packageName}/*' | wc -l | xargs )"`));
